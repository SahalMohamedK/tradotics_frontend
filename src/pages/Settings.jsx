...
function Settings() {
    ...
    let timeZoneInput = useRef()
    let currencyInput = useRef()


    function saveAccount(){
        setIsUpdateUser(true)
        let firstName = firstNameInput.current.getValue()
        let secondName = secondNameInput.current.getValue()
        let username = usernameInput.current.getValue()
        let email = emailInput.current.getValue()
        let phone = phoneInput.current.getValue()
        let timeZone = timeZoneInput.current.getValue()
        let currency = currencyInput.current.getValue()
        if(firstName && secondName && phone){
            updateUser({firstName, secondName, phone}).then(() => {
    ...
    function showAccount(){
        firstNameInput.current.setValue(user.firstName)
        secondNameInput.current.setValue(user.secondName)
        phoneInput.current.setValue(user.phone)
        emailInput.current.setValue(user.user)
    }

    useEffect(() => {
        if(isSigned === false){
            navigate('/signin')
        }else if(!isFirstSigned){
            showAccount()
            setIsLoading(false)
        }else{
            setIsLoading(false)
                    ...
                    <Tab id='account'>
                        <Card className='h-full'>
                            ...
                            <div className='md:flex md:space-x-2'>
                                <InputField ref={firstNameInput} className='w-full mb-3' label='First name' icon={fa1} 
                                    disabled={isUpdateUser} required/>
                                <InputField ref={secondNameInput} className='w-full mb-3' label='Last name' icon={fa2} 
                                    disabled={isUpdateUser} required/>
                            </div>                            
                            <div className='md:flex md:space-x-2'>
                                <InputField ref={usernameInput} className='w-full mb-3' label='Username' icon={faUser}
                                    disabled={isUpdateUser}/>
                                <InputField ref={emailInput} className='w-full mb-3' label='Email' icon={faEnvelope}
                                    disabled={isUpdateUser}/>
                            </div>                          
                            <div className='md:flex flex-wrap'>
                                <InputField ref={phoneInput} className='w-full md:w-1/3 mb-3 pr-2' label='Phone number' icon={faPhone} 
                                    disabled={isUpdateUser} required/>
                                <SelectField ref={timeZoneInput} className='w-full md:w-1/3 mb-3 pr-2' label='Time zone' icon={faClock} values={['Asia/Calcutta']}/>
                                <SelectField ref={currencyInput} className='w-full md:w-1/3 mb-3 lg:pr-0 md:pr-2' label='Currency' icon={faCoins} values={['INR - Indian rupees']}/>
                            </div>
                            <div className='flex space-x-2'>
